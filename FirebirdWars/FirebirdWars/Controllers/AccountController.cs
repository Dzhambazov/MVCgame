        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                ApplicationUser user = new ApplicationUser()
                {
                    ...
                    Population = 1,
                    MaxPopulation = 200,
                    LocationX = rand.Next(0, 500),
                    LocationY = rand.Next(0, 500)
                };
                ...
                if (result.Success)
                {
                    ...
                    {
                        var getUser = context.Users.FirstOrDefault(usr => usr.Id == user.Id);

                        var castle = context.Buildings.FirstOrDefault(b => b.Type == "Castle");
                        var lumberjacks = context.Buildings.FirstOrDefault(b => b.Type == "Lumberjacks");
                        var farm = context.Buildings.FirstOrDefault(b => b.Type == "Farm");
                        var ironMine = context.Buildings.FirstOrDefault(b => b.Type == "Iron Mine");

                        getUser.LastOnline = DateTime.Now;

                        context.UsersBuildings.Add(new UserBuilding { Building = castle, BuildingLevel = 1, User = getUser });
                        context.UsersBuildings.Add(new UserBuilding { Building = lumberjacks, BuildingLevel = 1, User = getUser });
                        context.UsersBuildings.Add(new UserBuilding { Building = farm, BuildingLevel = 1, User = getUser });
                        context.UsersBuildings.Add(new UserBuilding { Building = ironMine, BuildingLevel = 1, User = getUser });
